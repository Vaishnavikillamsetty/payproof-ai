def check_timeline_rules(case, evidence_list):
    ...
    if payment_evt:
        ...
        if payment_amount is not None and float(case.amount) != float(payment_amount):
            flags.append(("amount_mismatch", True, f"Disputed amount ({case.amount}) does not match payment record ({payment_amount})"))
    ...
    if case.dispute_reason == "unauthorized transaction" and otp_evt is not None:
        if otp_evt.content.get("verified") is True:
            flags.append(("unauthorized_but_otp_verified", True, "Transaction is disputed as unauthorized but OTP verification succeeded"))

    # 5. Duplicate charge without multiple payments
    if case.dispute_reason == "duplicate charge":
        if len(payment_evts) < 2:
            flags.append(("duplicate_charge_missing_evidence", True, "Claim is duplicate charge but multiple payment records were not found"))
    ...
    if case.dispute_reason == "subscription not cancelled":
        ...
        if not has_cancellation:
             flags.append(("cancellation_without_record", True, "Claim is subscription not cancelled but no cancellation communication found"))
